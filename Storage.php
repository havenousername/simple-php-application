...
abstract class FileIO implements IFileIO {
    protected string|false $filepath;
...
interface IStorage {
    ...
    function findAllIds($record, string $recordKey);
...
class Storage implements IStorage {
    ...
    public function findAllIds($record, string $recordKey): array
    {
        $keys = array();
        foreach ($this->contents as $key => $content) {
            if ($content[$recordKey] === $record) {
                $keys[] = $key;
            }
        }
